...
public final class MainCommand implements TabExecutor {
    ...
    private final List<String> IMPORT = new ArrayList<String>() {{
        add("import");
    }};
    private final List<String> RELOAD = new ArrayList<String>() {{
        add("reload");
    }};
    private final String[] msg = {ChatColor.AQUA + "[ItemsAPI] Help", "/iapi -- Show help;", "/iapi import -- Import item on main hand;", "/iapi reload -- Reload plugin.", "\n"};
                    ...
                    if (args[0].equals("import")) {
                        if (sender.hasPermission("itemsapi.import")) {
                            if (sender instanceof Player) {
                                ...
                                if (result)
                                    sender.sendMessage(ChatColor.GREEN + "Successfully imported item " + args[1]);
